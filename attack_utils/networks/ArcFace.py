...
class ArcFace1(Module):
    def __init__(self, **kwargs):
        super(ArcFace1, self).__init__()
        net = IR_50((112, 112))
        url = 'http://ml.cs.tsinghua.edu.cn/~xiaoyang/face_models/ArcFace/model_ir_se50.pth'
        self.channel = 'rgb'
        self.use_prelogits = False
        #embedding_size = kwargs.get('embedding_size', 512)
        no_gpu = kwargs.get('no_gpu', False)
        self.net, self.input_device = self.get_model(url, net, no_gpu)

    def get_model(self, url, net, no_gpu=False):
        """
